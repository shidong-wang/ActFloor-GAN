...
from models_guided import Generator_F2B, Generator_B2F
...
from datasets_rplan import ImageDataset
...
# Networks
netG_A2B = Generator_F2B(opt.input_nc, opt.output_nc)  #  floorplan -> boundary
netG_B2A = Generator_B2F(opt.output_nc, opt.input_nc)  #  boundary + activity -> floorplan
...
activity_C = Tensor(opt.batchSize, 1, opt.size, opt.size)
target_real = Variable(Tensor(opt.batchSize).fill_(1.0), requires_grad=False)
target_fake = Variable(Tensor(opt.batchSize).fill_(0.0), requires_grad=False)
mask_non_activity = Variable(Tensor(opt.batchSize, 1, opt.size, opt.size).fill_(-1.0), requires_grad=False) #-1.0 
...
for epoch in range(opt.epoch, opt.n_epochs):
	for i, batch in enumerate(dataloader):
		...
		activity_C = Variable(activity_C.copy_(batch['C']))
		...
		same_A = netG_B2A(real_A, mask_non_activity)
		loss_identity_A = criterion_identity(same_A, real_A) * 5.0  # ||Ga(a)-a||1

		# GAN loss
		fake_B = netG_A2B(real_A)
		pred_fake = netD_B(fake_B)
		loss_GAN_A2B = criterion_GAN(pred_fake.squeeze(), target_real.squeeze())  # log(Db(Gb(a)))

		fake_A = netG_B2A(real_B, activity_C)
		pred_fake = netD_A(fake_A)
		loss_GAN_B2A = criterion_GAN(pred_fake.squeeze(), target_real.squeeze())  # log(Da(Ga(b)))

		# Cycle loss
		recovered_A = netG_B2A(fake_B, activity_C) #
		...
		loss_D_real = criterion_GAN(pred_real.squeeze(), target_real.squeeze())  # log(Da(a))

		# Fake loss
		pred_fake = netD_A(fake_A.detach())
		loss_D_fake = criterion_GAN(pred_fake.squeeze(), target_fake.squeeze())  # log(1-Da(G(b)))
		...
		loss_D_real = criterion_GAN(pred_real.squeeze(), target_real.squeeze())  # log(Db(b))

		# Fake loss
		pred_fake = netD_B(fake_B.detach())
		loss_D_fake = criterion_GAN(pred_fake.squeeze(), target_fake.squeeze())  # log(1-Db(G(a)))
